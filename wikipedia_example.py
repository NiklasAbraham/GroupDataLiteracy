...
wiki_wiki = wikipediaapi.Wikipedia(user_agent='DataLiteracyProject (bartolmarkovinovic@gmail.com)', language='en')
movies = ["https://en.wikipedia.org/wiki/Inception", "/wiki/The_Matrix", "Interstellar (film)"]

for movie in movies:
    try:
        page = get_page_from_url(wiki_wiki, movie)
        print(f"Title: {page.title}")
        plot = get_plot_section(page)
        if plot:
            print(f"Plot (first 600 chars): {plot[:600]}...")
        else:
            print("Plot section not found.")
        print(f"Summary: {page.summary[0:60]}...")
        print()
