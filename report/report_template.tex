...
Previous approaches explore originality and temporal trends in movie plots by employing keyword search or topic modelling \cite{dubourg2023cinema}. While these older methods might be insightful but challenging, recent computational work \cite{xu2020cinderella} has revealed hidden cultural patterns in large narrative corpora. One such analysis conducted on the musical domain uses high dimensional embeddings to observe changes to structural properties over time \cite{DiMarco2025}. We build upon these foundations by leveraging advances in large language models (LLMs) to embed movie plot summaries \cite{sreenivasan2013quantitative} into a unified semantic space.
Using distances of these embeddings, we investigate relationships between movie genres across time, and develop a novelty score to quantify originality of movies. Furthermore, we are able to detect subtle shift in the movie narratives, e.g. a shift away from Spy (James Bond) like movies after the end of the Cold War.
