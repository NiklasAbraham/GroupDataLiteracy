...
We evaluated four main document embedding approaches to address these challenges. First, Mean Pooling computes the average of token embeddings to form the document vector, providing a simple aggregation method. Second, CLS Token uses the pretrained [CLS] token as a document summary, following standard transformer practice. Third, Early Chunk-then-Embed splits documents into chunks before embedding and averages their vectors, reducing length effects but limiting cross-chunk context. Fourth, Late Embed-then-Chunk embeds the full document first, then aggregates over sliding windows to capture broader context. Each method offers different tradeoffs between bias, variance, and preservation of semantic details.

To systematically compare these approaches, we evaluated ten embedding strategies on 5,000 movie plots using four key metrics. Length bias was measured through correlation between document length and embedding norm. Isotropy was assessed via variance in the first principal component. Genre classification accuracy evaluated semantic preservation. Class separation was quantified through silhouette scores and separation ratios (intra-class to inter-class similarity). The evaluation revealed substantial variation across methods: length-norm correlation ranged from -0.366 (Chunk-First 512/256) to 0.822 (Late Chunk 512/256), with CLS Token achieving near-zero correlation (0.004). Isotropy varied from 3.32\% (CLS Token) to 11.92\% (Late Chunk 512/256). Genre classification accuracy showed minimal differences (0.326 to 0.349), while separation ratios ranged from 0.943 to 0.961.

Given that no single method dominated across all dimensions, we selected CLS Token as our embedding strategy based on several practical considerations. CLS Token achieved the best length bias mitigation (correlation of 0.004), strong isotropy properties (3.32\% first-PC variance), and high separability (separation ratio of 0.958). Additionally, it requires no parameter tuning, represents the most standard approach in transformer-based embeddings, and maintains computational efficiency. While Mean Pooling and Late Chunking methods preserved more semantic detail, their strong length correlations (0.629--0.822) introduced systematic biases that could confound temporal analyses of semantic drift.
