...
# File is in src/001_data_pipline/, so go up one level to get src/
SRC_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if SRC_DIR not in sys.path:
    sys.path.insert(0, SRC_DIR)

# Import handlers
# Note: sys.path modification above is required for these imports
from api.moviedb_handler import (  # noqa: E402
    get_movie_by_id,
    get_movie_by_wiki_id,
    get_tmdb_api_key,
)
from api.wikidata_handler import fetch_movies_for_years  # noqa: E402
from api.wikipedia_handler import fetch_plot_from_url  # noqa: E402
from embedding.embedding import EmbeddingService  # noqa: E402
from embedding.util_embeddings import verify_embeddings, verify_gpu_setup  # noqa: E402
...
if not CHUNKING_AVAILABLE:
    logger.warning(
        "Chunking classes not available. Install required dependencies or check path."
    )
...
if TARGET_DEVICES is not None and all(
    isinstance(d, str) and d.startswith("cuda:") for d in TARGET_DEVICES
):
    ...
    if "CUDA_VISIBLE_DEVICES" not in os.environ:
        ...
        logger.info(
            f"Set CUDA_VISIBLE_DEVICES={cuda_visible_devices} to limit to devices: {TARGET_DEVICES}"
        )
...
def load_lexical_weights(
    year: int, chunking_suffix: str = "", data_dir: Optional[str] = None
    ...
    lexical_weights_path = os.path.join(
        data_dir, f"movie_lexical_weights_{year}{chunking_suffix}.npz"
    )
    ...
    try:
        ...
        token_indices_list = [
            token_indices_array[i] for i in range(len(token_indices_array))
        ]
        weights_list = [weights_array[i] for i in range(len(weights_array))]

        logger.info(
            f"Loaded lexical weights for year {year}: {len(token_indices_list)} documents"
        )
        logger.info(
            f"Total non-zero weights: {sum(len(ti) for ti in token_indices_list)}"
        )
...
def save_csv(df: pd.DataFrame, year: int) -> None:
    ...
    if "plot" in df.columns:
        df_to_save["plot"] = df_to_save["plot"].apply(
            lambda x: clean_plot_text(x) if pd.notna(x) else x
        )
...
async def step1_wikidata(
    ...
    for year in range(start_year, end_year + 1):
        ...
        if not force_refresh and os.path.exists(csv_path):
            logger.info(
                f"Year {year}: CSV file already exists, skipping Wikidata fetch..."
            )
        ...
        logger.info(
            f"Year {year}: CSV file not found, fetching movies from Wikidata..."
        )
...
async def step2_moviedb(
    year_dataframes: Dict[int, pd.DataFrame], verbose: bool = True
) -> Dict[int, pd.DataFrame]:
    """
    ...
    async with aiohttp.ClientSession() as session:
        for year, df in year_dataframes.items():
            ...
            if movies_to_process.empty:
                logger.info(
                    f"Year {year}: All {len(df)} movies already have TMDb data, skipping..."
                )
                enriched_dataframes[year] = df
                continue
                ...
                try:
                    ...
                    if tmdb_data and tmdb_data.get("movie_results"):
                        ...
                        if tmdb_id:
                            ...
                            if full_details:
                                df.at[idx, "popularity"] = full_details.get(
                                    "popularity"
                                )
                                df.at[idx, "vote_average"] = full_details.get(
                                    "vote_average"
                                )
                                df.at[idx, "vote_count"] = full_details.get(
                                    "vote_count"
                                )
                        ...
                        if verbose:
                            logger.debug(
                                f"Year {year}, {row.get('title', 'Unknown')}: No TMDb data found"
                            )
                    ...
                    logger.warning(
                        f"Year {year}, Movie {movie_id}: Error fetching TMDb data - {e}"
                    )
                    continue
...
def _process_single_movie_plot(
    idx: int, row: pd.Series, wikipedia_link: str, year: int, verbose: bool = True
    ...
    if error_msg and error_msg != "No plot found":
        ...
        logger.warning(
            f"Year {year}, Movie {row.get('title', 'Unknown')}: Error - {error_msg}"
        )
...
def step3_wikipedia(
    ...
    for year, df in year_dataframes.items():
        ...
        if force_refresh:
            ...
            if movies_to_process.empty:
                logger.info(
                    f"Year {year}: All {len(df)} movies already have plots, skipping..."
                )
                enriched_dataframes[year] = df
                continue
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_task = {
                executor.submit(
                    _process_single_movie_plot, idx, row, wikipedia_link, year, verbose
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    logger.error(
                        f"Year {year}, Movie {title}: Unexpected error in thread - {e}"
                    )
...
def step4_embeddings(
    ...
    try:
        ...
        if target_devices != ["cpu"] and all(
            d.startswith("cuda:") for d in target_devices
        ):
            import os
            ...
            if (
                original_cuda_visible is not None
                and original_cuda_visible != cuda_visible_devices
            ):
                logger.warning(
        ...
        logger.warning(
            f"Error during GPU setup verification: {e}. Continuing with available devices..."
        )
    ...
    try:
        embedding_service = EmbeddingService(
            model_name=model_name, target_devices=target_devices
        )
        _embedding_service_instance = embedding_service  # Store for cleanup
        logger.info(
            f"Initialized embedding service with model: {model_name} on devices: {target_devices}"
        )
    except Exception as e:
    ...
    if chunking_method and chunking_method.lower() not in ["none", ""]:
        if not CHUNKING_AVAILABLE:
            logger.error(
                "Chunking requested but chunking classes are not available. Skipping chunking."
            )
            ...
            try:
                if chunking_method_lower == "cls_token":
                    chunking_instance = CLSToken(
                        embedding_service=embedding_service, model_name=model_name
                    )
                    chunking_suffix = "_cls_token"
                    logger.info("Using chunking method: CLSToken")
                elif chunking_method_lower == "mean_pooling":
                    chunking_instance = MeanPooling(
                        embedding_service=embedding_service, model_name=model_name
                    )
                    ...
                    chunking_suffix = (
                        f"_chunk_first_{chunking_chunk_size}_{chunking_stride}"
                    )
                    logger.info(
                        f"Using chunking method: ChunkFirstEmbed (chunk_size={chunking_chunk_size}, stride={chunking_stride})"
                    )
                    ...
                    chunking_suffix = (
                        f"_late_chunking_{chunking_window_size}_{chunking_stride}"
                    )
                    logger.info(
                        f"Using chunking method: LateChunking (window_size={chunking_window_size}, stride={chunking_stride})"
                    )
                else:
                    logger.warning(
                        f"Unknown chunking method: {chunking_method}. Available options: 'cls_token', 'mean_pooling', 'chunk_first_embed', 'late_chunking'. Using direct embedding."
                    )
                    chunking_method = None
            except Exception as e:
                logger.error(
                    f"Error initializing chunking method {chunking_method}: {e}. Using direct embedding."
                )
                chunking_method = None
                chunking_instance = None
    ...
    for year, df in year_dataframes.items():
        ...
        if "movie_id" not in df.columns:
            logger.warning(
                f"Year {year}: No 'movie_id' column found, skipping embeddings"
            )
            continue
        ...
        embeddings_path = os.path.join(
            DATA_DIR, f"movie_embeddings_{year}{chunking_suffix}.npy"
        )
        movie_ids_path = os.path.join(
            DATA_DIR, f"movie_ids_{year}{chunking_suffix}.npy"
        )
        lexical_weights_path = (
            os.path.join(DATA_DIR, f"movie_lexical_weights_{year}{chunking_suffix}.npz")
            if save_lexical_weights
            else None
        )
        embeddings_exist = os.path.exists(embeddings_path) and os.path.exists(
            movie_ids_path
        )
        ...
        if embeddings_exist and not force_refresh:
            try:
                ...
                existing_embedding_count = (
                    existing_embeddings.shape[0] if existing_embeddings.ndim > 0 else 0
                )
                ...
                if current_plot_count == existing_embedding_count:
                    if len(existing_movie_ids) == len(movie_ids_with_plots):
                        if np.array_equal(
                            existing_movie_ids, np.array(movie_ids_with_plots)
                        ):
                            logger.info(
                                f"Year {year}: Embeddings file exists and matches current data ({current_plot_count} plots), skipping..."
                            )
                            continue
                        else:
                            logger.warning(
                ...
                logger.warning(
                    f"Year {year}: Error loading existing embeddings file: {e}. Regenerating..."
                )
        elif embeddings_exist and force_refresh:
            logger.info(
                f"Year {year}: force_refresh=True, regenerating embeddings even though file exists"
            )
        ...
        try:
            ...
            if chunking_instance is not None:
                ...
                if save_lexical_weights:
                    logger.info(
                        f"Year {year}: Extracting lexical weights separately..."
                    )
                    ...
                    for i in range(0, len(plots), batch_size):
                        ...
                        batch_results = embedding_service.encode_corpus(
                            batch_plots, batch_size=len(batch_plots)
                        )
                        if "lexical_weights" in batch_results:
                            ...
                            if isinstance(batch_lexical_weights, list):
                                ...
                                logger.warning(
                                    f"Year {year}: Unexpected lexical_weights format: {type(batch_lexical_weights)}"
                                )
                    if not lexical_weights_list:
                        logger.warning(
                            f"Year {year}: No lexical weights found in results. They may not be available for this model or chunking method."
                        )
            else:
                ...
                if save_lexical_weights:
                    ...
                    logger.info(
                        f"Year {year}: Encoding with lexical weights extraction..."
                    )
                    if "cpu" in target_devices or (
                        len(target_devices) == 1 and target_devices[0] == "cpu"
                    ):
                        # For CPU, we need to use encode_corpus
                        results = embedding_service.encode_corpus(
                            plots, batch_size=batch_size
                        )
                        ...
                        if "dense_vecs" in results:
                            ...
                            logger.warning(
                                f"Year {year}: Dense embeddings not found in results, using model.encode..."
                            )
                            embeddings = embedding_service.model.encode(
                                plots, batch_size=batch_size, show_progress_bar=verbose
                            )
                        ...
                        if "lexical_weights" in results:
                            ...
                            if not isinstance(lexical_weights_list, list):
                                logger.warning(
                                    f"Year {year}: lexical_weights is not a list: {type(lexical_weights_list)}"
                                )
                                lexical_weights_list = None
                    else:
                        ...
                        logger.info(
                            f"Year {year}: Encoding on {len(target_devices)} device(s): {target_devices}"
                        )
                        ...
                        if not lexical_weights_list:
                            logger.warning(
                                f"Year {year}: No lexical weights found in results."
                            )
                else:
            ...
            if verify_embeddings(embeddings, plots):
                ...
                logger.info(
                    f"Year {year}: Saved embeddings to {embeddings_path} (shape: {embeddings.shape})"
                )
                logger.info(
                    f"Year {year}: Saved movie_ids to {movie_ids_path} ({len(movie_ids_with_plots)} movie_ids)"
                )
                logger.info(
                    f"Year {year}: Encoding completed in {duration:.2f} seconds ({len(plots) / duration:.2f} docs/sec)"
                )
                logger.info(
                    f"Year {year}: Embeddings are indexed by movie_id - embedding[i] corresponds to movie_ids[i] (e.g., first embedding for movie_id: {movie_ids_with_plots[0] if movie_ids_with_plots else 'N/A'})"
                )

                # Save lexical weights if available
                if (
                    save_lexical_weights
                    and lexical_weights_list is not None
                    and len(lexical_weights_list) == len(movie_ids_with_plots)
                ):
                    try:
                        ...
                        for doc_idx, lw in enumerate(lexical_weights_list):
                            ...
                            if isinstance(lw, dict):
                                ...
                                for token_id_str, weight in lw.items():
                                    ...
                                    if (
                                        abs(weight_float) > 1e-10
                                    ):  # Only save non-zero weights
                                        try:
                                            ...
                                            logger.warning(
                                                f"Year {year}: Document {doc_idx}: Invalid token_id '{token_id_str}', skipping"
                                            )
                                ...
                                logger.warning(
                                    f"Year {year}: Document {doc_idx}: Unexpected lexical weight type: {type(lw)}"
                                )

                            # Convert to numpy arrays
                            token_indices_list.append(
                                np.array(token_indices, dtype=np.int64)
                                if token_indices
                        ...
                        avg_non_zero = (
                            total_non_zero / len(token_indices_list)
                            if token_indices_list
                            else 0
                        )
                        logger.info(
                            f"Year {year}: Saved lexical weights to {lexical_weights_path}"
                        )
                        logger.info(
                            f"Year {year}: Saved {len(lexical_weights_list)} lexical weight entries"
                        )
                        logger.info(
                            f"Year {year}: Total {total_non_zero} non-zero weights (avg {avg_non_zero:.1f} per document)"
                        )
                        logger.info(
                            f"Year {year}: Lexical weights are indexed by movie_id - token_indices[i] and weights[i] correspond to movie_ids[i]"
                        )
                    except Exception as e:
                    ...
                    if lexical_weights_list is None:
                        logger.warning(
                            f"Year {year}: Lexical weights requested but not available (model may not support them)"
                        )
                    elif len(lexical_weights_list) != len(movie_ids_with_plots):
                        logger.warning(
                            f"Year {year}: Lexical weights count mismatch: {len(lexical_weights_list)} vs {len(movie_ids_with_plots)}"
                        )
            else:
            ...
            logger.error(
                f"Year {year}: Error generating embeddings - {e}", exc_info=verbose
            )
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                if verbose:
                    logger.info(
                        f"Year {year}: GPU memory before clearing - allocated: {allocated_before:.2f} GB, reserved: {reserved_before:.2f} GB"
                    )
                ...
                if len(target_devices) > 1 and all(
                    d.startswith("cuda") for d in target_devices
                ):
                ...
                logger.info(
                    f"Year {year}: GPU memory cleared - allocated: {allocated_after:.2f} GB, reserved: {reserved_after:.2f} GB (freed {allocated_before - allocated_after:.2f} GB)"
                )
        except Exception as e:
...
async def run_pipeline(
    ...
    if existing_years:
        logger.info(
            f"Found existing CSV files for {len(existing_years)} years: {existing_years[:5]}{'...' if len(existing_years) > 5 else ''}"
        )
    if missing_years:
        logger.info(
            f"Missing CSV files for {len(missing_years)} years (will be fetched): {missing_years}"
        )
    logger.info("")
    ...
    if not skip_moviedb:
        year_dataframes = await step2_moviedb(
            year_dataframes=year_dataframes, verbose=verbose
        )
    else:
