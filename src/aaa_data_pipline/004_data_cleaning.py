...
# Project root (go up 3 levels from file location)
...
GENRE_TO_CLUSTER_ID_PATH = os.path.join(base_path, "src/config/genre_fix_mapping_new.json")
GENRE_CLUSTER_ID_TO_NAME_PATH = os.path.join(
    base_path, "src/config/cluster_to_genre_mapping_01122025.json"
)
...
def filter_movies_without_plot(df: pd.DataFrame) -> pd.DataFrame:
    filtered_df = df[df["plot"].notna() & (df["plot"].str.strip() != "")].reset_index(
        drop=True
    )
    return filtered_df
...
def filter_movies_with_single_occurrence_genres(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def remove_single_occurrence_genres(genre_str):
        if (
            pd.isna(genre_str)
            or not isinstance(genre_str, str)
            or not genre_str.strip()
        ):
            return genre_str
        genres = [g.strip() for g in genre_str.split(",") if g.strip()]
...
def add_token_features_columns(
...
def add_genres_clustered_columns(
    ...
    def map_genre_cluster_ids_to_names(cluster_id_str: str) -> str:
        if (
            pd.isna(cluster_id_str)
            or not isinstance(cluster_id_str, str)
            or cluster_id_str.strip() == ""
        ):
            return ""
    ...
    df["genre_cluster_names"] = df["genre_cluster_ids"].progress_apply(
        map_genre_cluster_ids_to_names
    )
    return df


def remove_movies_with_genre_cluster(
    df: pd.DataFrame, genre_cluster_id: str
) -> pd.DataFrame:
    def has_genre_cluster(genre_cluster_ids_str: str) -> bool:
        if pd.isna(genre_cluster_ids_str) or not isinstance(genre_cluster_ids_str, str):
            return False
        cluster_ids = {
            cid.strip() for cid in genre_cluster_ids_str.split(",") if cid.strip()
        }
    ...
    filtered_df = df[
        ~df["genre_cluster_ids"].progress_apply(has_genre_cluster)
    ].reset_index(drop=True)
    return filtered_df


def convert_string_solumns_nans_to_empty_strings(
    df: pd.DataFrame, string_columns: list
) -> pd.DataFrame:
...
async def clean_dataset(
    ...
    df_filtered = await filter_non_movies(
        df_filtered,
        wrong_classes_save_path=wrong_classes_save_path,
    ...
    if filter_single_genres:
        ...
        print(
            f"After removing single-occurrence genres from genre column: {len(df_filtered)}"
        )

    df_filtered["plot_length_chars"] = df_filtered["plot"].apply(
        lambda x: len(x.strip()) if isinstance(x, str) else 0
    )
    df_filtered = df_filtered[
        df_filtered["plot_length_chars"] <= max_plot_length
    ].reset_index(drop=True)
    print(
        f"After filtering movies with plot length > {max_plot_length} chars: {len(df_filtered)}"
    )

    print("Adding token features columns...")
    df_filtered = add_token_features_columns(df_filtered)

    df_filtered = df_filtered[
        df_filtered["token_shannon_entropy"] >= entropy_threshold
    ].reset_index(drop=True)
    print(
        f"After filtering movies with token Shannon entropy < {entropy_threshold}: {len(df_filtered)}"
    )

    if (
        genre_to_cluster_id_dict is not None
        and cluster_id_to_genre_name_dict is not None
    ):
        print("Adding genres clustered columns...")
        df_filtered = add_genres_clustered_columns(
            df_filtered, genre_to_cluster_id_dict, cluster_id_to_genre_name_dict
        )
        for genre_cluster_id in genre_cluster_ids_to_remove:
            df_filtered = remove_movies_with_genre_cluster(
                df_filtered, genre_cluster_id
            )
            print(
                f"After removing movies with genre cluster ID {genre_cluster_id}: {len(df_filtered)}"
            )
    else:
        print(
            "Skipping adding genres clustered columns and removing movies with specified genre cluster IDs due to missing mapping dictionaries."
        )

    print("Converting NaNs to empty strings in string columns...")
    df_filtered = convert_string_solumns_nans_to_empty_strings(
        df_filtered, string_columns_for_nan_to_empty
    )
