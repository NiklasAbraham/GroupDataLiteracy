...
from data_utils import load_movie_embeddings, load_movie_data, cluster_genres
...
def _normalize_genres(genres: np.ndarray) -> Tuple[List[List[str]], np.ndarray]:
    ...
    for idx, genre in enumerate(genres):
        ...
        if isinstance(genre, str):
            if '|' in genre:
                genre_list = [g.strip() for g in genre.split('|') if g.strip() and g.strip() != 'Unknown']
            else:
                genre_list = [g.strip() for g in genre.split(',') if g.strip()]
        elif isinstance(genre, list):
            genre_list = [g.strip() if isinstance(g, str) else str(g).strip() 
                         for g in genre if g and str(g).strip()]
        else:
            genre_str = str(genre)
            if '|' in genre_str:
                genre_list = [g.strip() for g in genre_str.split('|') if g.strip() and g.strip() != 'Unknown']
            else:
...
def analyze_genre_separability(
    ...
    for n_clusters in range(min_k, max_k + 1, step):
        ...
        if n_clusters % 10 == 0 or n_clusters == min_k:
            print(f"  k={n_clusters:2d}: Silhouette={silhouette:.3f}, ARI={ari:.3f}, NMI={nmi:.3f}, V-measure={v_measure:.3f}")
...
def analyze_genre_cluster_purity(
    ...
    cluster_purities = [(cid, info['purity'], info['size']) for cid, info in cluster_analysis.items()]
    ...
    print(f"  Best clusters (purity > 0.7):")
    for cid, purity, size in cluster_purities[:5]:
        if purity > 0.7:
            info = cluster_analysis[cid]
            print(f"    Cluster {cid}: {info['dominant_genre']} ({purity:.2%}, n={size})")
    
    print(f"  Worst clusters (purity < 0.3):")
    for cid, purity, size in cluster_purities[-5:]:
        if purity < 0.3:
            info = cluster_analysis[cid]
            print(f"    Cluster {cid}: {info['dominant_genre']} ({purity:.2%}, n={size})")
...
def analyze_genre_overlap_in_embedding_space(
    ...
    print(f"  Closest genre pairs (most similar in embedding space):")
    for genre1, genre2, dist in closest_pairs[:5]:
        print(f"    {genre1} <-> {genre2}: {dist:.3f}")
    
    print(f"  Most distant genre pairs (most distinct):")
    for genre1, genre2, dist in closest_pairs[-5:]:
        print(f"    {genre1} <-> {genre2}: {dist:.3f}")
...
def visualize_genre_clusters(
    ...
    if others_mask.sum() > 0:
        axes[1].scatter(embeddings_2d[others_mask, 0], embeddings_2d[others_mask, 1],
                       label='Other', alpha=0.3, s=5, c='gray')
...
def comprehensive_clustering_analysis(
    ...
    if separability_results and 'error' not in separability_results:
        best_k = max(separability_results.keys(), 
    ...
    if output_dir:
        ...
        visualize_genre_clusters(
            embeddings, genres, n_clusters=best_k, 
            output_path=str(viz_path), random_state=random_state
        )
    ...
    if 'purity' in results and 'overall_purity' in results['purity']:
        ...
        if overall_purity < 0.3:
            print("  WARNING: Very low cluster purity. Embeddings may not contain strong genre signals.")
            ...
            print("  MODERATE: Moderate cluster purity. Some genre information present.")
            print("  Recommendations:")
            ...
            print("  GOOD: High cluster purity. Embeddings contain strong genre signals.")
            print("  Recommendations:")
