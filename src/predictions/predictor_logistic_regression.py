...
) -> Union[Dict[str, float], Tuple[Dict[str, float], MultiOutputClassifier, MultiLabelBinarizer]]:
    """
...
def load_embeddings_and_genres(
    ...
    if verbose:
        print(f"Matched {len(combined_df)}/{len(embeddings_df)} embeddings with metadata")
    ...
    embedding_indices = [movie_id_to_idx[mid] for mid in matched_movie_ids if mid in movie_id_to_idx]
...
def main():
    ...
    if len(embeddings) == 0:
        print("ERROR: No embeddings loaded. Please check your data directory and chunking suffix.")
        return
