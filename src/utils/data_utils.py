...
_DEFAULT_DATA_DIR = str(_PROJECT_ROOT / "data" / "data_final")
_DEFAULT_CSV_PATH = str(_PROJECT_ROOT / "data" / "data_final" / "final_dataset.csv")


def load_embeddings_as_dict(
    data_dir: str, start_year: int = 1930, end_year: int = 2024
) -> Dict[str, np.ndarray]:
    """
    ...
    for year in range(start_year, end_year + 1):
        embeddings_path = os.path.join(
            data_dir, f"movie_embeddings_{year}_cls_token.npy"
        )
...
def load_movie_embeddings(
    ...
    """
    Loads all movie embeddings from year-based .npy files and returns them as arrays.
    ...
    for year in range(start_year, end_year + 1):
        ...
        if os.path.exists(embeddings_path) and os.path.exists(movie_ids_path):
            ...
            if verbose:
                print(f"Loaded {len(embeddings)} embeddings for year {year}")

    if len(all_embeddings) == 0:
        if verbose:
            print(
                f"No embeddings found in {data_dir} for years {start_year}-{end_year}"
            )
        return np.array([]), np.array([])

    # Concatenate all years
    embeddings_array = np.vstack(all_embeddings)
    movie_ids_array = np.concatenate(all_movie_ids)

    if verbose:
        print(f"Total loaded: {len(movie_ids_array)} embeddings")

    return embeddings_array, movie_ids_array
...
def load_final_dataset(csv_path: str = None, verbose: bool = False) -> pd.DataFrame:
    """
    ...
    Parameters:
    - csv_path: path to final_dataset.csv (defaults to data/data_final/final_dataset.csv)
    - verbose: whether to print statistics, default = False
...
def load_final_dense_embeddings(
    ...
    Parameters:
    - data_dir: directory containing the final embedding files (defaults to data/data_final)
    - verbose: whether to print statistics, default = False
...
def load_final_sparse_embeddings(
    ...
    Parameters:
    - data_dir: directory containing the final embedding file (defaults to data/data_final)
    - verbose: whether to print statistics, default = False
    ...
    try:
        ...
        if verbose:
            ...
            print(
                f"Average non-zero weights per document: {total_non_zero / len(token_indices_list):.1f}"
            )
...
def load_final_data_with_embeddings(
    csv_path: str = None, data_dir: str = None, verbose: bool = False
) -> pd.DataFrame:
    """
    ...
    Parameters:
    - csv_path: path to final_dataset.csv (defaults to data/data_final/final_dataset.csv)
    - data_dir: directory containing final embedding files (defaults to data/data_final)
    - verbose: whether to print statistics, default = False
...
def preprocess_genres(genre: str) -> str:
    ...
    # Read genre mapping file from config directory
    # Get the src directory (go up from utils/ to src/)
    src_dir = Path(__file__).parent.parent
    config_dir = src_dir / "config"
    mapping_file = config_dir / "genre_fix_mapping_new.json"

    # Fallback to old mapping if new one doesn't exist
    if not mapping_file.exists():
        mapping_file = config_dir / "genre_fix_mapping.json"
    ...
    for keyword in keywords:
        ...
        for col in search_columns:
            ...
            if case_sensitive:
                ...
                col_mask = col_data.str.contains(
                    keyword_clean, na=False, regex=False, case=False
                )
...
def keep_selected_top_genres(df: pd.DataFrame, top_indices: List[int]) -> pd.DataFrame:
    ...
    selected_genres = [
        genre_counts.index[i] for i in top_indices if i < len(genre_counts)
    ]
    # Filter dataframe
...
def filter_movies_with_given_genre(
    df: pd.DataFrame, genre_id: str, genre_id_column: str = "genre_cluster_ids"
) -> pd.DataFrame:
