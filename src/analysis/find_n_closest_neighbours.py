...
def find_n_closest_neighbours(
    ...
    valid_indices = np.array([i for i, mid in enumerate(all_movie_ids) if mid in valid_movie_ids])
    
    if len(valid_indices) == 0:
        raise ValueError(f"No movies found in the specified year range ({start_year}-{end_year})")
    ...
    logger.info(f"Filtered to {len(filtered_movie_ids)} movies with embeddings in year range")
    
    # Find the index of the query qid in the filtered data
    query_indices = np.where(filtered_movie_ids == qid)[0]
    if len(query_indices) == 0:
        raise ValueError(f"QID '{qid}' not found in embeddings for the specified year range")
    
    if len(query_indices) > 1:
        logger.warning(f"Multiple embeddings found for qid '{qid}', using first one")
    
    query_idx = query_indices[0]
    query_embedding = filtered_embeddings[query_idx:query_idx+1]  # Keep 2D shape for sklearn
