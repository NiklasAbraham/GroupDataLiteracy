...
def find_temporal_nearest_neighbors(df, k=10):
    ...
    for _, group in tqdm.tqdm(df_grouped_by_year):
        ...
        group_qids = movie_qids_sorted.iloc[current_idx : current_idx + n_samples].values.tolist()
        ...
        if index.ntotal > 0:
            ...
            results['neighbor_ids'].extend(
                movie_qids_sorted.iloc[neighbour_indices.flatten()].values.reshape(n_samples, k).tolist()
            )
...
if __name__ == "__main__":
    ...
    knn_results = find_temporal_nearest_neighbors(df, k=10)
