...
def main():
    ...
    exclude_qids = [
        ['Q1931001', 'Q101863729', 'Q7428298', 'Q7601132', 'Q108730566', 'Q1538082'],  # Exclude QIDs for 'Star Wars'
    ...
    for idx, keywords in enumerate(keywords_list):
        ...
        if keyword_exclude_qids:
            ...
            matching_qids = [qid for qid in matching_qids if qid not in keyword_exclude_qids]
            logger.info(f"After excluding {len(keyword_exclude_qids)} QIDs, {len(matching_qids)} movies remain (from {initial_count})")
        ...
        if len(matching_qids) > 0:
            ...
            matching_movies_info = movie_data[movie_data['movie_id'].isin(matching_qids)][['movie_id', 'title']].copy()
        ...
        if len(matching_embeddings) < 2:
            logger.warning(f"Only {len(matching_embeddings)} movies with embeddings found, need at least 2 for distance calculation")
            continue
        
        matching_embeddings = np.array(matching_embeddings)
        logger.info(f"Using {len(matching_embeddings)} movies with embeddings for analysis")
        
        # Calculate average distance within keyword group
        within_distance = calculate_average_cosine_distance(matching_embeddings)
        logger.info(f"Average cosine distance within keyword group: {within_distance:.4f}")
        
        # Sample random movies (excluding the keyword group movies)
        all_other_indices = [idx for idx, mid in enumerate(all_movie_ids) 
        ...
        random_indices = np.random.choice(all_other_indices, size=n_random, replace=False)
        ...
        logger.info(f"Average cosine distance vs {n_random} random movies: {vs_random_distance:.4f}")
    ...
    logger.info(f"Sampling {n_global_samples} random pairs for global average calculation...")
    ...
    if len(all_embeddings) <= 100:
        ...
        if len(all_pairs) > n_global_samples:
            pair_indices = np.random.choice(len(all_pairs), size=n_global_samples, replace=False)
    ...
    # Create bars
    bars1 = ax.bar(x - width/2, results['within_group_distance'], width, 
                   label='Within Group', color='steelblue', alpha=0.8)
    bars2 = ax.bar(x + width/2, results['vs_random_distance'], width,
                   label='Vs Random Movies', color='coral', alpha=0.8)
    
    # Add horizontal line for global average
    ax.axhline(y=global_avg_distance, color='green', linestyle='--', linewidth=2, 
               label=f'Global Average ({global_avg_distance:.3f})', alpha=0.8)
    ...
    ax.set_title('Average Cosine Distance: Within Keyword Groups vs Random Movies\n(Latent Embedding Space)', 
    ...
    for i, keyword in enumerate(results['keywords']):
        logger.info(f"\n{keyword}:")
        logger.info(f"  Within group distance: {results['within_group_distance'][i]:.4f}")
        ...
        logger.info(f"  Difference:            {diff:.4f} ({diff/results['within_group_distance'][i]*100:.1f}% higher)")
