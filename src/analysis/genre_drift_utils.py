...
def group_movies_by_columns(df: pd.DataFrame, columns_to_group: list, max_per_group: int = None, grouping_function: Callable[[Any], Any] = get_average_embedding, n_boostrap_samples: int = None) -> pd.DataFrame:
    """
...
def group_movies_by_columns(df: pd.DataFrame, columns_to_group: list, max_per_group: int = None,
                            ...
                            n_boostrap_samples: int = None) -> pd.DataFrame:
    """
    Groups movies by columns, calculating average embeddings per group and storing it in 'avg_embedding' column
    It reduces each movie group to one row
...
def calculate_drift_metrics(df: pd.DataFrame, n_years_per_group: int = 1, max_movies_per_group: int = None, grouping_function: Callable[[Any], Any] = get_average_embedding, n_boostrap_samples: int = None):
    """
    ...
    df = group_movies_by_columns(df, ["year_group", "new_genre"], max_per_group=max_movies_per_group, grouping_function=grouping_function, n_boostrap_samples=n_boostrap_samples)
