...
def print_genres_by_cluster(
    ...
    print_df = print_df[
        print_df["genre"].notna()
        & print_df["genre"].apply(lambda x: isinstance(x, str))
    ].copy()
        ...
        if cluster_label_mapping:
            ...
            try:
                ...
                if cluster_id in cluster_label_mapping:
                    display_label = (
                        f"Cluster {cluster_label} ({cluster_label_mapping[cluster_id]})"
                    )
                else:
...
def count_movies_by_cluster(
    ...
    mapping_json_path: str = "src/config/genre_fix_mapping_new.json",
    cluster_label_mapping: Optional[Dict[int, str]] = None,
) -> Dict[str, int]:
    """
    ...
    for _, row in movie_df.iterrows():
        ...
        movie_identifier = (
            row.get("movie_id")
            if "movie_id" in row and pd.notna(row.get("movie_id"))
            else row.name
        )
    ...
    for cluster_label in sorted_labels:
        ...
        if cluster_label_mapping:
            try:
                ...
                if cluster_id in cluster_label_mapping:
                    display_label = (
                        f"{cluster_label} ({cluster_label_mapping[cluster_id]})"
                    )
                else:
...
def plot_movie_counts_by_cluster(
    ...
    ax1.bar(
        ...
        total_counts,
        width,
        label="Total Count (with duplicates)",
        color="steelblue",
        alpha=0.7,
    )
    ...
    ax1.set_title(
        "Total Movie Counts by Cluster\n(including duplicates from multi-genre movies)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax2.bar(
        x_pos, unique_counts, width, label="Unique Movies", color="coral", alpha=0.7
    )
    ax2.set_xlabel("Cluster Label", fontsize=12)
    ax2.set_ylabel("Number of Unique Movies", fontsize=12)
    ax2.set_title(
        "Unique Movie Counts by Cluster\n(each movie counted once per cluster)",
        fontsize=14,
        fontweight="bold",
    )
...
def classify_genres(
    ...
    print_genres_by_cluster(
        genre_df_clustered, output_json_path, cluster_label_mapping, genre_to_id
    )
...
if __name__ == "__main__":
    ...
    descriptions_csv_path = os.path.join(
        base_path, "src", "analysis", "genre_description_df.csv"
    )
    ...
    print(
        f"Using {len(movie_df_for_counting)} movies for counting (after data cleaning)"
    )

    cluster_label_mapping = (
        None  # Can be set to a dict mapping cluster IDs to names if needed
    )
    cluster_counts, unique_movie_counts = count_movies_by_cluster(
        movie_df_for_counting, output_json_path, cluster_label_mapping
    )

    print_movie_counts_by_cluster(
        cluster_counts, unique_movie_counts, cluster_label_mapping
    )

    print("\nStep 10: Plotting movie counts by cluster...")
    plot_movie_counts_by_cluster(
        cluster_counts, unique_movie_counts, cluster_label_mapping
    )
