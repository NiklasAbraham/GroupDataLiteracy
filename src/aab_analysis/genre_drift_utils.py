...
from src.aab_analysis.chunking.calculations import (
    compute_scalar_difference,
    compute_vector_difference,
)
from src.aab_analysis.math_functions import (
    calculate_average_cosine_distance_between_groups,
)
from src.aab_analysis.math_functions.boostrapping_embeddings import (
    get_bootstrapped_embeddings,
)
from src.aab_analysis.math_functions.cosine_distance_util import get_average_embedding
from src.aab_analysis.normalization_strategy import (
    DriftNormalizationStrategy,
    NoNormalization,
)


def create_yearly_grouping_column(
    df: pd.DataFrame, n_years_per_group: int
) -> pd.DataFrame:
    """
...
def group_movies_by_columns(
    ...
    if n_boostrap_samples:
        ...
        def aggregation_func(x):
            return grouping_function(np.vstack(x))

    grouped_embeddings = (
        df.groupby(columns_to_group)["embedding"]
        .apply(lambda x: aggregation_func(x))
...
def calculate_bootstrapped_change(
    prev_embeddings: np.ndarray, next_embeddings: np.ndarray, change_func: Callable
) -> np.ndarray:
    """
...
def calculate_rate_of_change(
    df: pd.DataFrame, value_col: str, change_col_name: str, change_func
) -> pd.DataFrame:
    """
...
def calculate_movies_drift_velocity(
    df: pd.DataFrame, normalization_strategy: DriftNormalizationStrategy
):
    """
...
def cumsum_arrays(series):
    ...
    valid_arrays = [
        arr for arr in series.values if not (len(arr) == 1 and np.isnan(arr[0]))
    ]
...
def calculate_cumulative_change_of_column(
    df: pd.DataFrame, column_to_cum: str, columns_to_group_by: list[str]
) -> pd.DataFrame:
    """
    ...
    """
    df[f"{column_to_cum}_cum"] = df.groupby(columns_to_group_by)[
        column_to_cum
...
def calculate_drift_metrics(
    ...
    df = normalization_strategy.apply_preprocessing(
        df, columns_to_group=columns_to_group, random_seed=random_seed
    )

    df = group_movies_by_columns(
        df=df,
        columns_to_group=columns_to_group,
        grouping_function=grouping_function,
        n_boostrap_samples=n_boostrap_samples,
    )
    ...
    df = calculate_movies_drift_velocity(
        df=df, normalization_strategy=normalization_strategy
    )
...
def plot_dimension_averaged_across_genres(
    df: pd.DataFrame,
    embedding_col: str,
    dim_index: int,
    ...
    # Apply preprocessing
    df = normalization_strategy.apply_preprocessing(
        df=df, columns_to_group=["new_genre", "year_group"], random_seed=42
    )
    ...
    yearly_stats = normalization_strategy.normalize_embedding_values(
        df=df, embedding_col=embedding_col, dim_index=dim_index
    )
    ...
    plt.title(
        f"Embedding Dimension {dim_index} Averaged Across All Genres with {normalization_strategy.__str__()}"
    )
