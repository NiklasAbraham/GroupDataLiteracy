...
def filter_by_zipf(lemma2weight, zipf_threshold=2.5):
    ...
    """
    if not HAS_WORDFREQ:
        print("Warning: wordfreq not available, skipping Zipf filtering")
        return lemma2weight
...
def embed_and_save_concept_vocab(
    vocab, output_dir, model_name="BAAI/bge-small-en-v1.5", batch_size=512
):
    """
    ...
    vecs = model.encode(
        vocab, batch_size=batch_size, normalize_embeddings=True, show_progress_bar=True
    )
...
class ConceptSpace:
    ...
    def __init__(
        self, concept_words_path, concept_vecs_path, model_name="BAAI/bge-small-en-v1.5"
    ):
        self.concept_words = np.load(concept_words_path, allow_pickle=True).tolist()
        self.concept_vecs = np.load(concept_vecs_path).astype(
            np.float32
        )  # Shape: (N, d)
    ...
    def map_lemmas(self, lemma2weight, top_k=10):
        ...
        #   s[c*] += w_i
        if unknown_lemmas:
            unk_vecs = self.model.encode(
                unknown_lemmas, normalize_embeddings=True, show_progress_bar=False
            )
            unk_vecs = np.asarray(
                unk_vecs, dtype=np.float32
            )  # Shape: (U, d) where U = num unknown
...
def top_concept_nouns_embedding(
    lemma2weight, concept_embedder, concepts, concept_vecs, top_k=10
):
    """
    ...
    noun_vecs = concept_embedder.encode(
        noun_lemmas, normalize_embeddings=True, show_progress_bar=False
    )
    ...
    ranked = np.argsort(-concept_scores)
    return [
        (concepts[i], concept_scores[i])
        for i in ranked[:top_k]
        if concept_scores[i] > 0
    ]
...
def extract_top_concepts(
    ...
    noun_vecs = concept_embedder.encode(
        nouns, normalize_embeddings=True, show_progress_bar=False
    )
    noun_vecs = np.array(noun_vecs)
...
if __name__ == "__main__":
    ...
    if "dense_vecs" in results:
        print(
            f"dense_vecs type: {type(results['dense_vecs'])}, shape: {results['dense_vecs'].shape}"
        )
    ...
    if "lexical_weights" in results:
        ...
        if isinstance(lexical_weights, dict):
            ...
            try:
                ...
                lemma2weight = build_noun_lemma_weights(
                    plot_text, lexical_weights, tokenizer, nlp
                )
                ...
                if not lemma2weight:
                    ...
                    zipf_threshold = 4.0
                    filtered_lemma2weight = filter_by_zipf(
                        lemma2weight, zipf_threshold=zipf_threshold
                    )

                    print(
                        f"Filtered {len(lemma2weight)} lemmas to {len(filtered_lemma2weight)} with Zipf >= {zipf_threshold}"
                    )
                    ...
                    try:
                        ...
                        if top_concepts_wordnet_space:
                            print(f"\n{'=' * 80}")
                            print(
                                f"TOP 30 CONCEPTS (WORDNET 10K CONCEPT SPACE, Zipf >= {zipf_threshold}):"
                            )
                            print(f"{'=' * 80}")
                            for idx, (concept, score) in enumerate(
                                top_concepts_wordnet_space, 1
                            ):
        ...
        if isinstance(lexical_weights, np.ndarray):
            print(
                f"  lexical_weights[0] type: {type(lexical_weights)}, shape: {lexical_weights.shape}, dtype: {lexical_weights.dtype}"
            )
            ...
            for idx, (token_id, weight) in enumerate(zip(top_indices, top_weights), 1):
                try:
                    ...
                    print(
                        f"{idx:3d}. Token ID {token_id:6d}: '{token}' (weight: {weight:.6f})"
                    )
                except Exception:
                    print(
                        f"{idx:3d}. Token ID {token_id:6d}: <error converting> (weight: {weight:.6f})"
                    )
            print(f"{'=' * 80}\n")
        elif isinstance(lexical_weights, dict):
            print(
                f"  lexical_weights[0] type: dict, keys: {list(lexical_weights.keys())[:10]}... with a length of {len(lexical_weights)}"
            )
            sorted_items = sorted(
                lexical_weights.items(), key=lambda x: x[1], reverse=True
            )[:top_n]
            ...
            for idx, (token_id, weight) in enumerate(sorted_items, 1):
                try:
                    ...
                    print(
                        f"{idx:3d}. Token ID {token_id_int:6d}: '{token}' (weight: {weight:.6f})"
                    )
                except Exception:
                    token_id_int = (
                        int(token_id) if isinstance(token_id, str) else token_id
                    )
                    print(
                        f"{idx:3d}. Token ID {token_id_int:6d}: <error converting> (weight: {weight:.6f})"
                    )
            print(f"{'=' * 80}\n")
        elif isinstance(lexical_weights, (list, tuple)):
            print(
                f"  lexical_weights[0] type: {type(lexical_weights)}, length: {len(lexical_weights)}"
            )
            if len(lexical_weights) > 0:
                ...
                if isinstance(first_elem, np.ndarray):
                    ...
                    if len(lexical_weights) == 1:
                        ...
                        print(f"TOP {top_n} MAIN WORDS FROM LEXICAL WEIGHTS:")
                        print(f"{'=' * 80}")
                        for idx, (token_id, weight) in enumerate(
                            zip(top_indices, top_weights), 1
                        ):
                            try:
                                token = tokenizer.convert_ids_to_tokens(int(token_id))
                                print(
                    ...
                    sorted_items = sorted(
                        first_elem.items(), key=lambda x: x[1], reverse=True
                    )[:top_n]
            ...
            print(
                f"  lexical_weights[0] type: {type(lexical_weights)}, value: {lexical_weights}"
            )
